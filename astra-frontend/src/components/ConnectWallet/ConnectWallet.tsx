...
export const ConnectWallet = () => {
  ...
  const { publicKey, connected, connect, disconnect, signMessage, select, wallets } = useWallet()
  ...
  const handleSignMessage = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5156/api/v1/social/authenticate", {
        method: "POST",
          ...
          <div className="user-info">
            <div className="user-address">
              ...
              <span className="value">
                {user.defaultPublicKey.slice(0, 6)}...{user.defaultPublicKey.slice(-4)}
              </span>
            </div>
